...
class enrol_payu_testcase extends advanced_testcase {
    ...
    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('payu'));
        $plugin = enrol_get_plugin('payu');
        $this->assertInstanceOf('enrol_payu_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_payu', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $payuplugin = enrol_get_plugin('payu');

        // Just make sure the sync does not throw any errors when nothing to do.
        $payuplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_payu_plugin $payuplugin  */
        $payuplugin = enrol_get_plugin('payu');
        ...
        $payuplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $payuplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));

        $payuplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $payuplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
